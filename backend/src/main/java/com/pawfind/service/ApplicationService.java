...
public class ApplicationService {
        ...
        public ApplicationResponse submit(String email, Long petId, ApplicationRequest request) {
                ...
                if (adopter.getRole() != Role.ADOPTER) {
                        throw new IllegalStateException("Only adopters can submit applications");
                }
        ...
        public ApplicationResponse reject(String email, Long applicationId, String reason) {
                ...
                application.setRejectionReason(
                                reason != null && !reason.isBlank() ? reason : "Application was not approved");
                applicationRepository.save(application);

                // Pet goes back to AVAILABLE only if no other pending/approved application
                // exists for it
                boolean stillActive = applicationRepository.findByPetNgoId(application.getPet().getNgo().getId())
                                .stream()
                                .filter(a -> a.getPet().getId().equals(application.getPet().getId()))
                                .anyMatch(
        ...
        public ApplicationResponse signAgreement(String email, Long applicationId, String signatureDataUrl) {
                ...
                String pdfUrl = cloudinaryService.uploadPdf(pdfBytes, "pawfind/agreements",
